...
def _reread_with_edits(subtitle_file: SubtitleFile, draft_path: Path) -> SubtitleFile:
    ...
    for entry in parsed:
        ...
        if original is not None:
            # with_text() only sets `edited=True` when the text actually
            # differs (subtitles/models.py), so a re-save with no real
            # change leaves `edited` at its prior value instead of always
            # flipping it true.
            new_lines.append(original.with_text(entry.content))
        else:
            # A line the user added that has no originally generated
            # counterpart to compare against -- there is no "unedited"
            # value it could have, so it's edited by definition.
            new_lines.append(
                SubtitleLine(
                    index=entry.index,
