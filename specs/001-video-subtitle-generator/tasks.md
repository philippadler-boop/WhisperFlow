...
- [ ] T004 [P] Implement domain error types `UnsupportedVideoFormatError`, `MaxDurationExceededError`, `FfmpegNotFoundError` in `src/lib/errors.py` (spec FR-007; data-model.md validation rules)
- [ ] T005 [P] Implement `SubtitleLine` and `SubtitleFile` data classes plus `.srt` composition (via the `srt` library) in `src/subtitles/models.py` — shared by US1 (creates) and US2 (edits) (data-model.md — SubtitleLine/SubtitleFile; spec FR-006)
- [ ] T006 [P] Implement the `ProcessingJob` stage model (`ExtractingAudio` → `Transcribing` → `WritingSubtitles` → `AwaitingReview` → `Done`/`Failed`) and a stderr stage/percentage progress reporter in `src/cli/progress.py` (data-model.md — ProcessingJob; spec FR-011)
...
- [ ] T009 [P] [US1] Implement video probing (`container_format`, `duration_seconds`, `has_audio_track` via `ffmpeg`) in `src/audio/video_probe.py` (data-model.md — Video; spec FR-001, FR-007)
...
- [ ] T014 [US1] Wire the `--no-review` command path to the pipeline, with clear stderr error reporting (via T004's error types) for unsupported format, oversized video, or missing `ffmpeg` in `src/cli/main.py` (depends on T004, T007, T013) (spec FR-005, FR-007)
...
- [ ] T020 [US2] Implement the `--review` flow — write the draft `.srt`, invoke `$EDITOR`/`--editor`, wait for confirmation, re-read the file and mark changed lines' `edited` flag — in `src/cli/review.py` (depends on T005, T007) (data-model.md — SubtitleLine.edited; spec FR-009, FR-010)
- [ ] T021 [US2] Wire `--review`/`--no-review` branching into the CLI command, defaulting to `--review` per contracts/cli.md, in `src/cli/main.py` (depends on T014, T020) (spec FR-005)
- [ ] T022 [P] [US2] Integration test: `--review` with a simulated editor that changes one line → finalized `.srt` reflects the edit (quickstart.md Scenario 3) in `tests/integration/test_review.py`
...
- [ ] T025 [P] Add a benchmark helper (`scripts/benchmark.py`) that times a `transcribe` run against a given video and prints elapsed time, to support later SC-002/SC-006 verification (quickstart.md Scenario 8) — this produces timing evidence for QA, it does not itself constitute the validation report (constitution Principle I: subagent separation — validation reports are `qa`-owned)
...
- Polish: T024–T027 all touch different files and can run in parallel
...
- Per Principle IV, each task (or small group of related tasks) is implemented on its own feature branch off `001-video-subtitle-generator`, never committed directly to `main`.
